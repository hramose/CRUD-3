...
namespace Sone\CRUD\PanelTraits;

use Sone\CRUD\Exception\AccessDeniedException;
...
trait Access
{
    ...
    */
     ...
     * @return bool
     *
     * @throws \Sone\CRUD\Exception\AccessDeniedException in case the permission is not enabled
     */
    public function hasAccessOrFail($permission)
    {
        if (! in_array($permission, $this->access)) {
            throw new AccessDeniedException(trans('sone::crud.unauthorized_access', ['access' => $permission]));
        }
