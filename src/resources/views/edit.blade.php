		  ...
		  <div class="box">
		    <div class="box-header with-border">
		    	@if ($crud->model->translationEnabled())
					...
					<h3 class="box-title" style="line-height: 30px;">{{ trans('sone::crud.edit') }}</h3>
				@else
					<h3 class="box-title">{{ trans('sone::crud.edit') }}</h3>
				@endif
		    </div>
		    <div class="box-body row display-flex-wrap" style="display: flex;flex-wrap: wrap;">
		      <!-- load the view from the application if it exists, otherwise load the one in the package -->
		      @if(view()->exists('vendor.sone.crud.form_content'))
		      	@include('vendor.sone.crud.form_content', ['fields' => $fields, 'action' => 'edit'])
		      @else
