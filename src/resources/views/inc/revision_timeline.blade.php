...
@foreach($revisions as $revisionDate => $dateRevisions)
  <li class="time-label" data-date="{{ date('Y-m-d', strtotime($revisionDate)) }}">
      <span class="bg-red">
        {{ Date::parse($revisionDate)->format(config('sone.base.default_date_format')) }}
      </span>
  </li>
  ...
  <li class="timeline-item-wrap">
    ...
    <div class="timeline-item">
      ...
      @if($history->key == 'created_at' && !$history->old_value)
        <h3 class="timeline-header">{{ $history->userResponsible()?$history->userResponsible()->name:trans('sone::crud.guest_user') }} {{ trans('sone::crud.created_this') }} {{ $crud->entity_name }}</h3>
      @else
        <h3 class="timeline-header">{{ $history->userResponsible()?$history->userResponsible()->name:trans('sone::crud.guest_user') }} {{ trans('sone::crud.changed_the') }} {{ $history->fieldName() }}</h3>
        <div class="timeline-body p-b-0">
          <div class="row">
            <div class="col-md-6">{{ ucfirst(trans('sone::crud.from')) }}:</div>
            <div class="col-md-6">{{ ucfirst(trans('sone::crud.to')) }}:</div>
          </div>
          <div class="row">
        ...
        <div class="timeline-footer p-t-0">
          ...
          <button type="submit" class="btn btn-primary btn-sm restore-btn" data-entry-id="{{ $entry->id }}" data-revision-id="{{ $history->id }}" onclick="onRestoreClick(event)">
            <i class="fa fa-undo"></i> {{ trans('sone::crud.undo') }}</button>
          </form>
        </div>
      @endif
    </div>
    ...
    function onRestoreClick(e) {
      ...
      $.ajax('{{ \Request::url().'/' }}' +  revisionId + '/restore', {
        ...
        success: function(revisionTimeline) {
          ...
          new PNotify({
              text: '{{ trans('sone::crud.revision_restored') }}',
              type: 'success'
          });
        }
      });
