...
@if ($crud->checkIfFieldIsFirstOfItsType($field))
    ...
    @push('crud_fields_scripts')
        <script src="{{ asset('vendor/sone/ckeditor/ckeditor.js') }}"></script>
        <script src="{{ asset('vendor/sone/ckeditor/adapters/jquery.js') }}"></script>
    @endpush
...
<script>
    jQuery(document).ready(function($) {
        $('#ckeditor-{{ $field['name'] }}').ckeditor({
            "filebrowserBrowseUrl": "{{ url(config('sone.base.route_prefix').'/elfinder/ckeditor') }}",
