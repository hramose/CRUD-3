...
trait SaveActions
{
    ...
    {
        $saveAction = session('save_action', config('sone.crud.default_save_action', 'save_and_back'));
    ...
    public function setSaveAction($forceSaveAction = null)
    {
        $saveAction = $forceSaveAction ?:
            \Request::input('save_action', config('sone.crud.default_save_action', 'save_and_back'));

        if (config('sone.crud.show_save_action_change', true) &&
            session('save_action', 'save_and_back') !== $saveAction) {
            \Alert::info(trans('sone::crud.save_action_changed_notification'))->flash();
        }
    ...
    {
        $saveAction = \Request::input('save_action', config('sone.crud.default_save_action', 'save_and_back'));
    ...
    private function getSaveActionButtonName($actionValue = 'save_and_back')
    {
        switch ($actionValue) {
            case 'save_and_edit':
                return trans('sone::crud.save_action_save_and_edit');
                break;
            case 'save_and_new':
                return trans('sone::crud.save_action_save_and_new');
                break;
            case 'save_and_back':
            default:
                return trans('sone::crud.save_action_save_and_back');
                break;
        }
    }
}
