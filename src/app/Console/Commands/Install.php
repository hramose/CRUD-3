...
namespace Sone\CRUD\app\Console\Commands;

use Illuminate\Console\Command;
use Symfony\Component\Process\Process;
use Sone\Base\app\Console\Commands\Install as BaseInstall;
...
class Install extends BaseInstall
{
     ...
     * @var string
     */
    protected $signature = 'sone:crud:install
     ...
     * @var string
     */
    protected $description = 'Make uploads directory and publish assets for Sone\CRUD dependencies';
    ...
    {
        /*
        * "ask" comes by default, when no option provided, like: "sone:crud:install"
        ...
        */
        ...
        $this->info(" Sone\CRUD installation started. Please wait...");
        ...
        if ($install_elfinder) {
            ...
            $this->executeProcess('php artisan vendor:publish --provider="Sone\CRUD\CrudServiceProvider" --tag="elfinder"');

            $this->line(' Adding sidebar menu item for File Manager');
            switch (DIRECTORY_SEPARATOR) {
                case '/': // unix
                    $this->executeProcess('php artisan sone:base:add-sidebar-content "<li><a href=\"{{ sone_url(\'elfinder\') }}\"><i class=\"fa fa-files-o\"></i> <span>{{ trans(\'sone::crud.file_manager\') }}</span></a></li>"');
                    break;
                case '\\': // windows
                    $this->executeProcess('php artisan sone:base:add-sidebar-content "<li><a href=""{{ sone_url(\'elfinder\') }}""><i class=""fa fa-files-o""></i> <span>{{ trans(\'sone::crud.file_manager\') }}</span></a></li>"');
                    break;
        ...
        $this->executeProcess('php artisan vendor:publish --provider="Sone\CRUD\CrudServiceProvider" --tag="public"');

        $this->line(' Publishing CRUD language files');
        $this->executeProcess('php artisan vendor:publish --provider="Sone\CRUD\CrudServiceProvider" --tag="lang"');

        $this->line(' Publishing CRUD config file and custom elFinder config file');
        $this->executeProcess('php artisan vendor:publish --provider="Sone\CRUD\CrudServiceProvider" --tag="config"');

        $this->progressBar->finish();
        $this->info(" Sone\CRUD installation finished.");
    }
}
